...
namespace LibProShip.Infrastructure.Unpickling
{
	...
	public class Pickler : IDisposable
    {
        ...
        protected static readonly IDictionary<Type, IObjectPickler> customPicklers =
            new Dictionary<Type, IObjectPickler>();
        ...
        private bool dispatch(Type t, object o)
        {
            ...
            // check registry
            var custompickler = getCustomPickler(t);
        ...
        private void put_arrayOfPrimitives(Type t, object array)
        {
            ...
            if (t == typeof(byte))
            {
                ...
                outs.Write(output, 0, output.Length);
                var str = PickleUtils.rawStringFromBytes((byte[]) array);
                put_string(str);
            ...
            if (t == typeof(sbyte))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var s in (sbyte[]) array) save(s);
            }
            else if (t == typeof(short))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var s in (short[]) array) save(s);
            }
            else if (t == typeof(ushort))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var s in (ushort[]) array) save(s);
            }
            else if (t == typeof(int))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var i in (int[]) array) save(i);
            }
            else if (t == typeof(uint))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var i in (uint[]) array) save(i);
            }
            else if (t == typeof(long))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var v in (long[]) array) save(v);
            }
            else if (t == typeof(ulong))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var v in (ulong[]) array) save(v);
            }
            else if (t == typeof(float))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var f in (float[]) array) save(f);
            }
            else if (t == typeof(double))
            {
                ...
                outs.WriteByte(Opcodes.EMPTY_LIST);
                outs.WriteByte(Opcodes.MARK);
                foreach (var d in (double[]) array) save(d);
            }
        ...
        private void put_long(long v)
        {
            ...
            // 4-byte signed int?
            var high_bits = v >> 31; // shift sign extends
