...
namespace LibProShip.Infrastructure.Unpickling.Objects
{
    ...
    public class DateTimeConstructor : IObjectConstructor
    {
        ...
        private static DateTime CreateDateTime(dynamic[] args)
        {
            ...
            if (args[0] is string)
            {
                var parameters = (string) args[0];
                if (parameters.Length != 10)
                    throw new PickleException(
                        "invalid pickle data for datetime; expected arg of length 10, got length " + parameters.Length);
                yhi = parameters[0];
                ylo = parameters[1];
                month = parameters[2];
                day = parameters[3];
            ...
            else
            {
                ...
                var ms1 = parameters[7] & 0xff;
                var ms2 = parameters[8] & 0xff;
                var ms3 = parameters[9] & 0xff;
                microsec = (((ms1 << 8) | ms2) << 8) | ms3;
            }
        ...
        private static TimeSpan CreateTime(dynamic[] args)
        {
            ...
            if (args[0] is string)
            {
                ...
                if (parameters.Length != 6)
                    throw new PickleException("invalid pickle data for time; expected arg of length 6, got length " +
                                              parameters.Length);
                hour = parameters[0];
                minute = parameters[1];
                second = parameters[2];
            ...
            else
            {
                ...
                if (parameters.Length != 6)
                    throw new PickleException(
                        "invalid pickle data for datetime; expected arg of length 6, got length " + parameters.Length);
                hour = parameters[0] & 0xff;
                minute = parameters[1] & 0xff;
                second = parameters[2] & 0xff;
                var ms1 = parameters[3] & 0xff;
                var ms2 = parameters[4] & 0xff;
                var ms3 = parameters[5] & 0xff;
                microsec = (((ms1 << 8) | ms2) << 8) | ms3;
            }
        ...
        private static DateTime CreateDate(dynamic[] args)
        {
            ...
            if (args[0] is string)
            {
                ...
                if (parameters.Length != 4)
                    throw new PickleException("invalid pickle data for date; expected arg of length 4, got length " +
                                              parameters.Length);
                yhi = parameters[0];
                ylo = parameters[1];
                month = parameters[2];
