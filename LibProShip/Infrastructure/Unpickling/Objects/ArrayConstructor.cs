    ...
    public class ArrayConstructor : IObjectConstructor
    {
        public dynamic construct(dynamic[] args)
        {
            ...
            if (args.Length == 4)
            {
                ...
                var arraytype = ((string) args[1])[0];
                var machinecodeType = (int) args[2];
            ...
            var values = args[1] as ArrayList;
            if (values == null) throw new PickleException("unsupported Python 2.6 array pickle format");
            ...
            switch (typecode[0])
            {
                ...
                case 'u': // Unicode character 2 -> char[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = ((string) c)[0];
                    return result;
                }
                case 'b': // signed char -> sbyte[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToSByte(c);
                    return result;
                }
                case 'B': // unsigned char -> byte[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToByte(c);
                    return result;
                }
                case 'h': // signed short -> short[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToInt16(c);
                    return result;
                }
                case 'H': // unsigned short -> ushort[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToUInt16(c);
                    return result;
                }
                case 'i': // signed integer -> int[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToInt32(c);
                    return result;
                }
                case 'I': // unsigned integer 4 -> uint[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToUInt32(c);
                    return result;
                }
                case 'l': // signed long -> long[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToInt64(c);
                    return result;
                }
                case 'L': // unsigned long -> ulong[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToUInt64(c);
                    return result;
                }
                case 'f': // floating point 4 -> float[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToSingle(c);
                    return result;
                }
                case 'd': // floating point 8 -> double[]
                {
                    ...
                    var i = 0;
                    foreach (var c in values) result[i++] = Convert.ToDouble(c);
                    return result;
                }
                default:
            ...
            switch (typecode)
            {
                ...
                case 'l': // signed integer 4/8 -> int[]/long[]
                {
                    ...
                    if ((machinecode == 12 || machinecode == 13) && data.Length % 8 != 0)
                        throw new PickleException("data size alignment error");
                    if (machinecode == 8 || machinecode == 9) return constructIntArrayFromInt32(machinecode, data);
                ...
                case 'L': // unsigned integer 4/8 -> uint[]/ulong[]
                {
                    ...
                    if ((machinecode == 10 || machinecode == 11) && data.Length % 8 != 0)
                        throw new PickleException("data size alignment error");
                    if (machinecode == 6 || machinecode == 7) return constructUIntArrayFromUInt32(machinecode, data);
        ...
        protected char[] constructCharArrayUTF32(int machinecode, byte[] data)
        {
            ...
            for (var index = 0; index < data.Length / 4; ++index)
                if (machinecode == 20)
                {
                    var codepoint = PickleUtils.bytes_to_integer(data, index * 4, 4);
                    var cc = char.ConvertFromUtf32(codepoint);
                    if (cc.Length > 1)
                ...
                else
                {
                    ...
                    var codepoint = PickleUtils.bytes_to_integer(bigendian);
                    var cc = char.ConvertFromUtf32(codepoint);
                    if (cc.Length > 1)
                        throw new PickleException("cannot process UTF-32 character codepoint " + codepoint);
                    result[index] = cc[0];
